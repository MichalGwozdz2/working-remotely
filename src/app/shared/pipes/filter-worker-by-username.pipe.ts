...
@Pipe({
  name: 'filterWorkerByUsername'
})
export class FilterWorkerByUsernamePipe implements PipeTransform, OnDestroy {
  ...
  constructor(private store: Store<fromRemote.State>) {
    this.workStatusesState = this.store.select('remote')
      .pipe(
        map((state: fromRemote.FilterWorkerUsername) => state.filterWorkerUsername))
